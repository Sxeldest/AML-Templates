...
BitStream::BitStream(const BitStream& rhs)
{
	...
	if (copyData) {
		...
		size_t bytes = SAMP_BITS_TO_BYTES(rhs.numberOfBitsAllocated);

		if (bytes <= haveBytes) {
			data = stackData;
      memcpy(data, rhs.data, bytes);
      numberOfBitsAllocated = SAMP_BITS_TO_BYTES(SAMP_BITSTREAM_STACK_ALLOCATION_SIZE);
		} else {
...
BitStream& BitStream::operator=(const BitStream& rhs)
{
	...
	if (copyData) {
		size_t haveBytes = SAMP_BITS_TO_BYTES(numberOfBitsAllocated);
		size_t bytes = SAMP_BITS_TO_BYTES(rhs.numberOfBitsAllocated);

		if (bytes <= haveBytes) {
			memcpy(data, rhs.data, bytes);
		} else {
			if (data && numberOfBitsAllocated > SAMP_BYTES_TO_BITS(SAMP_BITSTREAM_STACK_ALLOCATION_SIZE)) {
				free(data);
			}
			data = (unsigned char*)malloc(bytes);
			memcpy(data, rhs.data, bytes);
...
void BitStream::Reset( void )
{
	...
	{
		//  memset(data, 0, SAMP_BITS_TO_BYTES(numberOfBitsUsed));
	}
	...
	//  memset(data, 0, SAMP_BITS_TO_BYTES(numberOfBitsAllocated));
}
...
void BitStream::Write( const char* input, const int numberOfBytes )
{
	...
	{
		AddBitsAndReallocate( SAMP_BYTES_TO_BITS(numberOfBytes) );
		memcpy(data+SAMP_BITS_TO_BYTES(numberOfBitsUsed), input, numberOfBytes);
		numberOfBitsUsed+=SAMP_BYTES_TO_BITS(numberOfBytes);
	}
	else
	{
...
bool BitStream::ReadBits( unsigned char* output, int numberOfBitsToRead, const bool alignBitsToRight )
{
	...
	memset( output, 0, SAMP_BITS_TO_BYTES( numberOfBitsToRead ) );
...
//		int newByteOffset = SAMP_BITS_TO_BYTES( numberOfBitsAllocated );
		// Use realloc and free so we are more efficient than delete and new for resizing
		int amountToAllocate = SAMP_BITS_TO_BYTES( newNumberOfBitsAllocated );
		if (data==(unsigned char*)stackData)
		{
			 if (amountToAllocate > SAMP_BITSTREAM_STACK_ALLOCATION_SIZE)
			 {
				 ...
				 memcpy ((void *)data, (void *)stackData, SAMP_BITS_TO_BYTES( numberOfBitsAllocated )); 
			 }
		}
		else
		{
...
void BitStream::PrintBits( void ) const
{
	...
	for ( int counter = 0; counter < SAMP_BITS_TO_BYTES( numberOfBitsUsed ); counter++ )
	{
		int stop;
...
int BitStream::CopyData( unsigned char** _data ) const
{
	...
	*_data = new unsigned char [ SAMP_BITS_TO_BYTES( numberOfBitsUsed ) ];
	memcpy( *_data, data, sizeof(unsigned char) * ( SAMP_BITS_TO_BYTES( numberOfBitsUsed ) ) );
...
{
	return SAMP_BITS_TO_BYTES( numberOfBitsUsed );
}
...
void BitStream::AssertCopyData( void )
{
	if ( copyData == false )
	{
		...
		if (numberOfBitsAllocated > 0) {
			size_t bytes = SAMP_BITS_TO_BYTES(numberOfBitsAllocated);
			unsigned char* newdata;

			if (bytes < SAMP_BITSTREAM_STACK_ALLOCATION_SIZE) {
				newdata = stackData;
			} else {
				newdata = (unsigned char*)malloc(bytes);
			}
			...
			memcpy( newdata, data, SAMP_BITS_TO_BYTES( numberOfBitsAllocated ) );
			data = newdata;
		} else {
			data = nullptr;
		}
...
{
	// quick and dirty fix to not include WSA2
	return false;
}
