...
namespace RakNet
{
	...
	public:
		...
		/// There is no benefit to calling this, unless you know exactly how many bytes you need and it is greater than SAMP_BITSTREAM_STACK_ALLOCATION_SIZE.
    ...
    /// Copy Constructor (MonetLoader)
    BitStream(const BitStream& rhs);

    /// Copy Assignment Operator (MonetLoader)
    BitStream& operator=(const BitStream& rhs);
		...
		/// Bidirectional serialize/deserialize any integral type to/from a bitstream.  Undefine SAMP_BITSTREAM_NATIVE_END if you need endian swapping.
		...
		/// Bidirectional serialize/deserialize any integral type to/from a bitstream.  Undefine SAMP_BITSTREAM_NATIVE_END if you need endian swapping.
		/// If you are not using SAMP_BITSTREAM_NATIVE_END the opposite is true for types larger than 1 byte
		...
		/// Write any integral type to a bitstream.  Undefine SAMP_BITSTREAM_NATIVE_END if you need endian swapping.
		/// \param[in] var The value to write
		template <class templateType>
		...
		/// Read any integral type from a bitstream.  Define SAMP_BITSTREAM_NATIVE_END if you need endian swapping.
		...
		/// Read any integral type from a bitstream.  Undefine SAMP_BITSTREAM_NATIVE_END if you need endian swapping.
		/// For floating point, this is lossy, using 2 bytes for a float and 4 for a double.  The range must be between -1 and +1.
		/// For non-floating point, this is lossless, but only has benefit if you use less than half the range of the type
		/// If you are not using SAMP_BITSTREAM_NATIVE_END the opposite is true for types larger than 1 byte
		/// \param[in] var The value to read
		...
		inline int GetNumberOfBytesUsed( void ) const {return SAMP_BITS_TO_BYTES( numberOfBitsUsed );}
	...
	private:
		...
		/// BitStreams that use less than SAMP_BITSTREAM_STACK_ALLOCATION_SIZE use the stack, rather than the heap to store data.  It switches over if SAMP_BITSTREAM_STACK_ALLOCATION_SIZE is exceeded
