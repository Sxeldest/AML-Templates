...
RakNet::StringCompressor* RakNet::StringCompressor::instance = NULL;
int RakNet::StringCompressor::referenceCount = 0;

void RakNet::StringCompressor::AddReference(void)
{
	if (++referenceCount==1)
	{
...
void RakNet::StringCompressor::RemoveReference(void)
{
...
RakNet::StringCompressor* RakNet::StringCompressor::Instance(void)
{
	return instance;
}
...
{
	RakNetDataStructures::Map<int, RakNetDataStructures::HuffmanEncodingTree *>::IMPLEMENT_DEFAULT_COMPARISON();

	// Make a default tree immediately, since this is used for RPC possibly from multiple threads at the same time
	RakNetDataStructures::HuffmanEncodingTree *huffmanEncodingTree = new RakNetDataStructures::HuffmanEncodingTree;
	huffmanEncodingTree->GenerateFromFrequencyTable( englishCharacterFrequencies );
	huffmanEncodingTrees.Set(0, huffmanEncodingTree);
}
void RakNet::StringCompressor::GenerateTreeFromStrings( unsigned char *input, unsigned inputLength, int languageID )
{
	...
	// Build the tree
	huffmanEncodingTree = new RakNetDataStructures::HuffmanEncodingTree;
...
void RakNet::StringCompressor::EncodeString( const char *input, int maxCharsToWrite, RakNet::BitStream *output, int languageID )
{
	RakNetDataStructures::HuffmanEncodingTree *huffmanEncodingTree;
...
bool RakNet::StringCompressor::DecodeString( char *output, int maxCharsToWrite, RakNet::BitStream *input, int languageID )
{
	RakNetDataStructures::HuffmanEncodingTree *huffmanEncodingTree;
	if (huffmanEncodingTrees.Has(languageID)==false)
		return false;
